...
const DesktopLayoutContainer: FC<PropsWithChildren> = ({ children }) => {
  ...
  const innerCssVariables = useMemo<Record<string, string>>(
    () => ({
      '--container-border-color': isDark ? theme.colorBorderSecondary : theme.colorBorder,
      '--container-border-radius': `${theme.borderRadiusLG}px`,
    }),
    [isDark, theme.colorBorderSecondary, theme.colorBorder, theme.borderRadiusLG],
  )

  return (
    <Flexbox className={containerStyles.outerContainer} height={'100%'} width={'100%'}>
      <Flexbox
