...
const CloudflareCard = ({
  ...
  const { cloudflareRunning, cloudflareLoading, cloudflareUrl } = useServiceStore()
  const { startCloudflare, stopCloudflare } = useServiceStore()
  const { cloudflareTunnelToken, cloudflareTunnelPublicUrl, saveSettings } = useSettingsStore()
    ...
    <>
      <Form form={form} layout="vertical" style={{ marginBottom: 12 }}>
        ...
        {cfToken && (
          ...
          >
            <Input style={{ maxWidth: 400 }} placeholder="https://my-app.example.com" />
          </Form.Item>
        )}
      </Form>
