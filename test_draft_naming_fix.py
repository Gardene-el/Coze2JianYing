...
"""
测试草稿命名修复
验证使用项目名称而非 UUID 作为文件夹名可以避免剪映重命名问题
"""
...
def test_draft_naming_with_project_name():
    ...
    try:
        ...
        expected_folder_name = "测试项目"
        
        # 检查文件夹名称是否为项目名称而非 UUID
...
def test_draft_naming_with_default_name():
    ...
    try:
        ...
        draft_path = draft_paths[0]
        expected_folder_name = "Coze剪映项目"  # 默认名称
...
def test_draft_naming_with_uuid_comparison():
    """对比测试：使用 UUID vs 项目名称"""
    print("\n=== 对比测试：文件夹命名方式 ===\n")
    
    # 测试使用 UUID 的情况（旧方式）
    print("旧方式: 使用 UUID 作为文件夹名")
    print("问题: 剪映可能会重命名 UUID 文件夹，导致 save() 失败\n")
    
    # 测试使用项目名称的情况（新方式）
    print("新方式: 使用项目名称作为文件夹名")
    print("优势: 人类可读，剪映不会重命名，save() 可正常工作\n")
...
if __name__ == "__main__":
    ...
    # 运行测试
    results.append(("使用项目名称", test_draft_naming_with_project_name()))
    results.append(("使用默认名称", test_draft_naming_with_default_name()))
