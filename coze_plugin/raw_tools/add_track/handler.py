...
自动从 API 端点生成: /{draft_id}/add_track
...
class Input(NamedTuple):
    """add_track 工具的输入参数"""
    draft_id: str
    track_type: Optional[str] = Ellipsis
    track_name: Optional[Any] = None
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_track，参数: {args.input}")
...
draft_id_{generated_uuid} = "{generated_uuid}"
        ...
        # 构造 request 对象
        req_{generated_uuid} = AddTrackRequest(track_type=args.input.track_type, track_name=args.input.track_name)

resp_{generated_uuid} = await add_track(draft_id_{generated_uuid}, req_{generated_uuid})
"""
        ...
        error_msg = f"调用 add_track 时发生错误: {str(e)}"
        if logger:
