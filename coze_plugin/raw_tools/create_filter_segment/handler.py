"""
create_filter_segment 工具处理器

自动从 API 端点生成: /filter/create
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 create_filter_segment，参数: {args.input}")
    ...
    # 生成 API 调用代码
    api_call = f"""
# API 调用: create_filter_segment
# 时间: {time.strftime('%Y-%m-%d %H:%M:%S')}

    # 构造 request 对象
    req_{generated_uuid} = CreateFilterSegmentRequest(filter_type=args.input.filter_type, target_timerange=args.input.target_timerange, intensity=args.input.intensity)
...
"""
    ...
    append_api_call_to_file(coze_file, api_call)
        ...
        if logger:
            logger.info(f"create_filter_segment 调用成功")
        ...
        error_msg = f"调用 create_filter_segment 时发生错误: {str(e)}"
        if logger:
