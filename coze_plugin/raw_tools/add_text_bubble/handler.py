"""
add_text_bubble 工具处理器

自动从 API 端点生成: /text/{segment_id}/add_bubble
...
class Input(NamedTuple):
    """add_text_bubble 工具的输入参数"""
    segment_id: str  # 片段ID
    effect_id: str  # 气泡特效 ID
    resource_id: str  # 资源 ID


# Output 类型定义
class Output(NamedTuple):
    """add_text_bubble 工具的输出参数"""
    success: bool = False  # 是否成功
    bubble_id: str = ""  # 气泡 UUID，错误时为空字符串
    message: str = ""  # 响应消息
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_text_bubble，参数: {args.input}")
    ...
    try:
        ...
        # 生成 API 调用代码
        api_call = f"""
# API 调用: add_text_bubble
# 时间: {time.strftime('%Y-%m-%d %H:%M:%S')}

# 构造 request 对象
req_params_{generated_uuid} = {{}}
req_params_{generated_uuid}['effect_id'] = "{args.input.effect_id}"
...
"""
        ...
        if logger:
            logger.info(f"add_text_bubble 调用成功")

        return Output(success=True, bubble_id="", message="操作成功", error_code=None, category=None, level=None, details=None, api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_text_bubble 时发生错误: {str(e)}"
        if logger:
