"""
add_video_fade 工具处理器

自动从 API 端点生成: /video/{segment_id}/add_fade
...
class Input(NamedTuple):
    """add_video_fade 工具的输入参数"""
    segment_id: str
    in_duration: Optional[str] = Ellipsis
    out_duration: Optional[str] = Ellipsis
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_video_fade，参数: {args.input}")
    ...
    try:
        ...
        # 生成 API 调用代码
        api_call = f"""
# API 调用: add_video_fade
# 时间: {time.strftime('%Y-%m-%d %H:%M:%S')}

segment_id_{generated_uuid} = "{generated_uuid}"

        # 构造 request 对象
        req_{generated_uuid} = AddFadeRequest(in_duration=args.input.in_duration, out_duration=args.input.out_duration)

resp_{generated_uuid} = await add_video_fade(segment_id_{generated_uuid}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_video_fade 调用成功")
        ...
        error_msg = f"调用 add_video_fade 时发生错误: {str(e)}"
        if logger:
