...
## 工具介绍
此工具对应 FastAPI 端点: `/video/{segment_id}/add_background_filling`

为视频片段添加转场效果, 可配置转场类型及时长 转场将应用于片段与下一片段的衔接处
...
| fill_type | 填充类型: blur 或 color | str | 是 |
| blur | 模糊程度（fill_type=blur 时） | Optional[float] | 否 |
| color | 填充颜色（fill_type=color 时） | Optional[str] | 否 |
