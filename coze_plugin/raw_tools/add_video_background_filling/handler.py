"""
add_video_background_filling 工具处理器

自动从 API 端点生成: /video/{segment_id}/add_background_filling
...
class Input(NamedTuple):
    """add_video_background_filling 工具的输入参数"""
    segment_id: str  # 片段ID
    fill_type: str  # 填充类型: blur 或 color
    blur: Optional[float] = 0.0625  # 模糊程度（fill_type=blur 时）
    color: Optional[str] = "#00000000"  # 填充颜色（fill_type=color 时）


# Output 类型定义
class Output(NamedTuple):
    """add_video_background_filling 工具的输出参数"""
...
def handler(args: Args[Input]) -> Output:
    """
    add_video_background_filling 的主处理函数
    ...
    if logger:
        logger.info(f"调用 add_video_background_filling，参数: {args.input}")
...
req_{generated_uuid} = AddBackgroundFillingRequest(**req_params_{generated_uuid})
