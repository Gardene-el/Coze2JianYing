"""
add_text_animation 工具处理器

自动从 API 端点生成: /text/{segment_id}/add_animation
...
class Input(NamedTuple):
    """add_text_animation 工具的输入参数"""
    segment_id: str  # 片段ID
    animation_type: str  # 动画类型: TextAnimationType
    duration: Optional[str] = "1s"  # 动画时长


# Output 类型定义
class Output(NamedTuple):
    """add_text_animation 工具的输出参数"""
    success: bool = False  # 是否成功
    animation_id: str = ""  # 动画 UUID，错误时为空字符串
    message: str = ""  # 响应消息
...
def handler(args: Args[Input]) -> Output:
    ...
    if logger:
        logger.info(f"调用 add_text_animation，参数: {args.input}")
...
resp_raw_{generated_uuid} = await add_text_animation(segment_{args.input.segment_id}, req_{generated_uuid})
resp_{generated_uuid} = AddTextAnimationResponse(**resp_raw_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_text_animation 调用成功")

        return Output(success=True, animation_id="", message="操作成功", error_code=None, category=None, level=None, details=None)

    except Exception as e:
        error_msg = f"调用 add_text_animation 时发生错误: {str(e)}"
        if logger:
