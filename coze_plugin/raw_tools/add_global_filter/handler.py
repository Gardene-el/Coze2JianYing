"""
add_global_filter 工具处理器

自动从 API 端点生成: /{draft_id}/add_filter
...
class Input(NamedTuple):
    """add_global_filter 工具的输入参数"""
    draft_id: str
    filter_type: Optional[str] = Ellipsis
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_global_filter，参数: {args.input}")
    ...
    try:
        ...
        # 生成 API 调用代码
        api_call = f"""
# API 调用: add_global_filter
# 时间: {time.strftime('%Y-%m-%d %H:%M:%S')}

draft_id_{generated_uuid} = "{generated_uuid}"

        # 构造 request 对象
        req_{generated_uuid} = AddGlobalFilterRequest(filter_type=args.input.filter_type, target_timerange=args.input.target_timerange, intensity=args.input.intensity)

resp_{generated_uuid} = await add_global_filter(draft_id_{generated_uuid}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_global_filter 调用成功")
        ...
        error_msg = f"调用 add_global_filter 时发生错误: {str(e)}"
        if logger:
