"""
add_video_effect 工具处理器

自动从 API 端点生成: /video/{segment_id}/add_effect
...
class Input(NamedTuple):
    """add_video_effect 工具的输入参数"""
    segment_id: str  # 片段ID
    effect_type: str  # 视频特效类型: VideoSceneEffectType | VideoCharacterEffectType
    params: Optional[List[float]] = None  # 特效参数列表


# Output 类型定义
class Output(NamedTuple):
    """add_video_effect 工具的输出参数"""
...
def handler(args: Args[Input]) -> Output:
    ...
    if logger:
        logger.info(f"调用 add_video_effect，参数: {args.input}")
...
resp_raw_{generated_uuid} = await add_video_effect(segment_{args.input.segment_id}, req_{generated_uuid})
resp_{generated_uuid} = AddVideoEffectResponse(**resp_raw_{generated_uuid})
"""
