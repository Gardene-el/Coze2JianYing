"""
add_audio_fade 工具处理器

自动从 API 端点生成: /audio/{segment_id}/add_fade
...
class Input(NamedTuple):
    """add_audio_fade 工具的输入参数"""
    segment_id: str  # 片段ID
    in_duration: str  # 淡入时长（字符串如 '1s' 或微秒数）
    out_duration: str  # 淡出时长


# Output 类型定义
class Output(NamedTuple):
    """add_audio_fade 工具的输出参数"""
...
def handler(args: Args[Input]) -> Output:
    ...
    if logger:
        logger.info(f"调用 add_audio_fade，参数: {args.input}")
...
req_params_{generated_uuid}['in_duration'] = "{args.input.in_duration}"
req_params_{generated_uuid}['out_duration'] = "{args.input.out_duration}"
req_{generated_uuid} = AddAudioFadeRequest(**req_params_{generated_uuid})

resp_raw_{generated_uuid} = await add_audio_fade(segment_{args.input.segment_id}, req_{generated_uuid})
resp_{generated_uuid} = AddAudioFadeResponse(**resp_raw_{generated_uuid})
"""
