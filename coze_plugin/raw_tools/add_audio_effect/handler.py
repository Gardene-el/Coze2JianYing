"""
add_audio_effect 工具处理器

自动从 API 端点生成: /audio/{segment_id}/add_effect
...
class Input(NamedTuple):
    """add_audio_effect 工具的输入参数"""
    segment_id: str  # 片段ID
    effect_type: str  # 音效类型: AudioSceneEffectType | ToneEffectType | SpeechToSongType
    params: Optional[List[float]] = None  # 特效参数列表（范围 0-100）


# Output 类型定义
class Output(NamedTuple):
    """add_audio_effect 工具的输出参数"""
    success: bool = False  # 是否成功
    effect_id: str = ""  # 特效 UUID，错误时为空字符串
    message: str = ""  # 响应消息
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_audio_effect，参数: {args.input}")
...
req_{generated_uuid} = AddAudioEffectRequest(**req_params_{generated_uuid})

resp_{generated_uuid} = await add_audio_effect(segment_{args.input.segment_id}, req_{generated_uuid})
"""
        ...
        return Output(success=True, effect_id="", message="操作成功", error_code=None, category=None, level=None, details=None, api_call=api_call)._asdict()
