"""
add_video_animation 工具处理器

自动从 API 端点生成: /video/{segment_id}/add_animation
...
class Input(NamedTuple):
    """add_video_animation 工具的输入参数"""
    segment_id: str  # 片段ID
    animation_type: str  # 动画类型: IntroType | OutroType | GroupAnimationType
    duration: Optional[str] = "1s"  # 动画时长


# Output 类型定义
class Output(NamedTuple):
    """add_video_animation 工具的输出参数"""
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_video_animation，参数: {args.input}")
...
"""
        ...
        if logger:
            logger.info(f"add_video_animation 调用成功")

        return Output(success=True, animation_id="", message="操作成功", error_code=None, category=None, level=None, details=None, api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_video_animation 时发生错误: {str(e)}"
        if logger:
