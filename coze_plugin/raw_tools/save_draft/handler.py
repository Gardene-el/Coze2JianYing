...
class Output(NamedTuple):
    ...
    draft_path: str = ""  # 草稿文件夹路径，错误时为空字符串
    message: str = ""  # 响应消息
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 save_draft，参数: {args.input}")
...
"""
        ...
        if logger:
            logger.info(f"save_draft 调用成功")

        return Output(success=True, draft_path="", message="操作成功", error_code=None, category=None, level=None, details=None)._asdict()

    except Exception as e:
        error_msg = f"调用 save_draft 时发生错误: {str(e)}"
        if logger:
