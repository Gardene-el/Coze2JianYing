"""
add_video_transition 工具处理器

自动从 API 端点生成: /video/{segment_id}/add_transition
...
class Input(NamedTuple):
    """add_video_transition 工具的输入参数"""
    segment_id: str  # 片段ID
    transition_type: str  # 转场类型
    duration: Optional[str] = "1s"  # 转场时长


# Output 类型定义
class Output(NamedTuple):
    """add_video_transition 工具的输出参数"""
    success: bool = False  # 是否成功
    transition_id: str = ""  # 转场 UUID，错误时为空字符串
    message: str = ""  # 响应消息
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_video_transition，参数: {args.input}")
    ...
    try:
        ...
        # 生成 API 调用代码
        api_call = f"""
# API 调用: add_video_transition
# 时间: {time.strftime('%Y-%m-%d %H:%M:%S')}

# 构造 request 对象
req_params_{generated_uuid} = {{}}
req_params_{generated_uuid}['transition_type'] = "{args.input.transition_type}"
...
"""
        ...
        if logger:
            logger.info(f"add_video_transition 调用成功")

        return Output(success=True, transition_id="", message="操作成功", error_code=None, category=None, level=None, details=None, api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_video_transition 时发生错误: {str(e)}"
        if logger:
