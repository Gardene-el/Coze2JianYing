"""
add_audio_keyframe 工具处理器

自动从 API 端点生成: /audio/{segment_id}/add_keyframe
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_audio_keyframe，参数: {args.input}")
...
resp_{generated_uuid} = await add_audio_keyframe(segment_id_{generated_uuid}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_audio_keyframe 调用成功")
        ...
        error_msg = f"调用 add_audio_keyframe 时发生错误: {str(e)}"
        if logger:
