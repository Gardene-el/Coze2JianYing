...
| 维度 | 得分 | 评价 |
|------|------|------|
| 参数完整性 | 95/100 | 覆盖 95% pyJianYingDraft 参数 |
...
- `change_pitch` (中等优先级) - 变速时保持音调
- `volume` for video (中等优先级) - 视频音量控制
...
- 参数完整性: 95%（覆盖了 95% 的 pyJianYingDraft 参数）
- 已识别遗漏: change_pitch, video volume, flip_*
- 结论: 可以满足绝大多数需求，少量参数可后续补充
