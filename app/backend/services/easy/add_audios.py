...
from app.backend.exceptions import CustomException, CustomError
import os
...
def add_audio_track(script: draft.ScriptFile) -> str:
    """添加音频轨道到草稿"""
    track_name = f"audio_track_{uuid.uuid4().hex[:12]}"
...
def find_audio_effect_type(audio_effect: str):
    ...
    if effect_type is None:
        for effect_name, effect_meta in draft.VideoSceneEffectType.__members__.items():
            if effect_meta.value.name == audio_effect:
                effect_type = effect_meta
                break
    ...
    if effect_type is None:
        for effect_name, effect_meta in draft.VideoCharacterEffectType.__members__.items():
            if effect_meta.value.name == audio_effect:
                effect_type = effect_meta
                break
...
def create_audio_segment(audio_path: str, start_time: int, segment_duration: int, audio: dict):
    ...
    audio_segment = draft.AudioSegment(
        material=audio_path,
        target_timerange=draft.trange(start=start_time, duration=segment_duration),
        volume=audio['volume']
    )
    return audio_segment


def add_segment_with_overlap_handling(script: draft.ScriptFile, track_name: str, audio_segment, audio_path: str, start_time: int, segment_duration: int, audio: dict):
    """添加片段到轨道，处理可能的重叠问题"""
    try:
            ...
            while attempts < max_attempts:
                try:
                    ...
                    adjusted_audio_segment = draft.AudioSegment(
                        material=audio_path,
                        target_timerange=draft.trange(start=adjusted_start, duration=segment_duration),
                        volume=audio['volume']
                    )
...
def validate_numeric_ranges(processed_item: Dict[str, Any]):
    ...
    if processed_item["duration"] is not None and processed_item["duration"] <= 0:
        ...
        raise CustomException(CustomError.INVALID_AUDIO_INFO, "duration must be greater than 0")