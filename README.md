...
Coze2JianYing 提供三种使用模式，满足不同场景需求：

#### 模式 1：手动粘贴模式（最简单）

适合快速测试和偶尔使用。

1. 在 Coze 工作流中生成 JSON 数据
2. 复制 JSON 数据
3. 打开草稿生成器 GUI
4. 粘贴 JSON 并点击生成
...
适合频繁使用，需要本地运行 API 服务。

1. 启动草稿生成器的本地 API 服务
2. 使用内网穿透工具（如 ngrok）暴露服务
3. 在 Coze 中配置云侧插件（基于服务）
4. Coze 工作流自动调用 API 生成草稿

📖 **详细指南**：[Coze 集成指南](./docs/guides/COZE_INTEGRATION_GUIDE.md)

#### 模式 3：云端 API 模式（生产级）

适合团队协作和长期使用。

1. 将 API 服务部署到云服务器或 Serverless 平台
2. 在 Coze 中配置云侧插件（基于服务）
3. 享受完全自动化的端到端工作流

📖 **部署指南**：[Coze 集成指南 - 云端部署](./docs/guides/COZE_INTEGRATION_GUIDE.md#方式二云端部署推荐用于生产)
