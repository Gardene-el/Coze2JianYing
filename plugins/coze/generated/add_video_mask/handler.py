...
class Input(NamedTuple):
    ...
    segment_id: str  # 片段ID
    mask_type: str  # 蒙版类型（MaskType 枚举名）
    center_x: Optional[float] = 0.0  # 蒙版中心 X 坐标（以素材像素为单位）
    center_y: Optional[float] = 0.0  # 蒙版中心 Y 坐标（以素材像素为单位）
    size: Optional[float] = 0.5  # 蒙版主尺寸（占素材高度比例）
    feather: Optional[float] = 0.0  # 羽化程度 0-100
    invert: Optional[bool] = False  # 是否反转蒙版
    rotation: Optional[float] = 0.0  # 蒙版顺时针旋转角度
    rect_width: Optional[float] = None  # 矩形蒙版宽度（占素材宽度比例），仅在 mask_type=矩形 时生效
    round_corner: Optional[float] = None  # 矩形蒙版圆角 0-100，仅在 mask_type=矩形 时生效
...
if {args.input.rect_width} is not None:
    req_params_{generated_uuid}['rect_width'] = {args.input.rect_width}
