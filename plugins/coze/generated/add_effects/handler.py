...
class Input(NamedTuple):
    """add_effects 工具的输入参数"""
    draft_id: str  # 草稿ID
    effect_infos: str  # 特效信息列表，JSON字符串


# Output 类型定义
class Output(NamedTuple):
    """add_effects 工具的输出参数"""
    segment_ids: List[str] = []  # 特效片段ID列表
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_effects，参数: {args.input}")
...
req_params_{generated_uuid}['effect_infos'] = "{args.input.effect_infos}"
req_{generated_uuid} = AddEffectsRequest(**req_params_{generated_uuid})

resp_{generated_uuid} = await add_effects(draft_{args.input.draft_id}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_effects 调用成功")

        return Output(segment_ids=[], api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_effects 时发生错误: {str(e)}"
        if logger:
