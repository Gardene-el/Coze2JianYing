"""
create_text_segment 工具处理器

自动从 API 端点生成: 
源文件: D:/Codespace/coze-jianying/Coze2JianYing/src/backend/api/basic.py
"""
...
class Input(NamedTuple):
    ...
    text_content: str = "据说pyJianYingDraft效果还不错?"  # 文本内容
    target_timerange: TimeRange  # 在轨道上的时间范围
    font_family: Optional[str] = "文轩体"  # 字体名称
    text_style: Optional[TextStyle] = None  # 文本样式（字体大小、颜色、加粗等）
    text_border: Optional[TextBorder] = None  # 文本描边，None 表示无描边
    text_shadow: Optional[TextShadow] = None  # 文本阴影，None 表示无阴影
    text_background: Optional[TextBackground] = None  # 文本背景，None 表示无背景
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 create_text_segment，参数: {args.input}")
...
if {args.input.text_content} is not None:
...
if {_is_meaningful_object(args.input.text_border)}:
    req_params_{generated_uuid}['text_border'] = {_to_type_constructor(args.input.text_border, 'TextBorder')}
if {_is_meaningful_object(args.input.text_shadow)}:
    req_params_{generated_uuid}['text_shadow'] = {_to_type_constructor(args.input.text_shadow, 'TextShadow')}
if {_is_meaningful_object(args.input.text_background)}:
    req_params_{generated_uuid}['text_background'] = {_to_type_constructor(args.input.text_background, 'TextBackground')}
...
"""
        ...
        return Output(segment_id=f"{generated_uuid}")._asdict()
