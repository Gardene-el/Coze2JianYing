...
class Input(NamedTuple):
    """add_masks 工具的输入参数"""
    draft_id: str  # 草稿ID
    segment_ids: List[str] = []  # 要应用遮罩的片段ID数组
    name: str = "线性"  # 遮罩类型名称
    X: int = 0  # 遮罩中心X坐标（像素）
    Y: int = 0  # 遮罩中心Y坐标（像素）
    width: int = 512  # 遮罩宽度（像素）
    height: int = 512  # 遮罩高度（像素）
    feather: int = 0  # 羽化程度（0-100）
    rotation: int = 0  # 旋转角度（度）
    invert: bool = False  # 是否反转遮罩
    round_corner: int = 0  # 圆角半径（0-100）
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_masks，参数: {args.input}")
...
if {args.input.segment_ids} is not None:
    req_params_{generated_uuid}['segment_ids'] = "{args.input.segment_ids}"
if {args.input.name} is not None:
    req_params_{generated_uuid}['name'] = "{args.input.name}"
if {args.input.X} is not None:
    req_params_{generated_uuid}['X'] = {args.input.X}
if {args.input.Y} is not None:
    req_params_{generated_uuid}['Y'] = {args.input.Y}
...
if {args.input.round_corner} is not None:
    req_params_{generated_uuid}['round_corner'] = {args.input.round_corner}
req_{generated_uuid} = AddMasksRequest(**req_params_{generated_uuid})

resp_{generated_uuid} = await add_masks(draft_{args.input.draft_id}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_masks 调用成功")

        return Output(api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_masks 时发生错误: {str(e)}"
        if logger:
