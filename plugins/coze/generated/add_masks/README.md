...
* **segment\_ids** (List\[str], optional): 要应用遮罩的片段ID数组
* **name** (str, optional): 遮罩类型名称
* **X** (int, optional): 遮罩中心X坐标（像素）
* **Y** (int, optional): 遮罩中心Y坐标（像素）
* **width** (int, optional): 遮罩宽度（像素）
* **height** (int, optional): 遮罩高度（像素）
* **feather** (int, optional): 羽化程度（0-100）
* **rotation** (int, optional): 旋转角度（度）
* **invert** (bool, optional): 是否反转遮罩
* **round\_corner** (int, optional): 圆角半径（0-100）
