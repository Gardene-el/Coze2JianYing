...
class Input(NamedTuple):
    """add_sticker_keyframe 工具的输入参数"""
    segment_id: str  # 片段ID
    time_offset: int  # 时间偏移量，单位：微秒（1秒 = 1,000,000微秒）
    value: float  # 关键帧值
    property: str  # KeyframeProperty 枚举成员，格式：KeyframeProperty.<name>，可选属性：KeyframeProperty.position_x / KeyframeProperty.position_y / KeyframeProperty.rotation / KeyframeProperty.scale_x / KeyframeProperty.scale_y / KeyframeProperty.uniform_scale


# Output 类型定义
class Output(NamedTuple):
    """add_sticker_keyframe 工具的输出参数"""
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_sticker_keyframe，参数: {args.input}")
...
req_{generated_uuid} = AddStickerKeyframeRequest(**req_params_{generated_uuid})

resp_{generated_uuid} = await add_sticker_keyframe(segment_{args.input.segment_id}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_sticker_keyframe 调用成功")

        return Output(api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_sticker_keyframe 时发生错误: {str(e)}"
        if logger:
