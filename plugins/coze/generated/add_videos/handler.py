...
class Input(NamedTuple):
    """add_videos 工具的输入参数"""
    draft_id: str  # 草稿ID
    video_infos: str  # 视频信息列表，JSON字符串
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_videos，参数: {args.input}")
...
req_params_{generated_uuid}['video_infos'] = "{args.input.video_infos}"
...
req_{generated_uuid} = AddVideosRequest(**req_params_{generated_uuid})

resp_{generated_uuid} = await add_videos(draft_{args.input.draft_id}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_videos 调用成功")

        return Output(segment_ids=[], api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_videos 时发生错误: {str(e)}"
        if logger:
