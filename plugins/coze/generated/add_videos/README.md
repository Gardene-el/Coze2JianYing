...
* **video\_infos** (str, required): 视频信息列表，JSON字符串
* **alpha** (float, optional): 全局透明度\[0, 1]
* **scale\_x** (float, optional): X轴缩放比例
* **scale\_y** (float, optional): Y轴缩放比例
* **transform\_x** (int, optional): X轴位置偏移(像素)
* **transform\_y** (int, optional): Y轴位置偏移(像素)

## 输出参数

* **segment\_ids** (List\[str]): 片段ID列表
