...
class Input(NamedTuple):
    ...
    material_url: str = "https://gardene-el.github.io/Coze2JianYing/assets/audio.mp3"  # 音频素材 URL
    target_timerange: TimeRange  # 在轨道上的时间范围
    source_timerange: Optional[TimeRange] = None  # 素材裁剪范围
    speed: float = 1.0  # 播放速度
    volume: float = 0.6  # 音量 0-2
    change_pitch: bool = True  # 是否跟随变速改变音调


# Output 类型定义
class Output(NamedTuple):
    """create_audio_segment 工具的输出参数"""
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 create_audio_segment，参数: {args.input}")
