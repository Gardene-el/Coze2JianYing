...
class Input(NamedTuple):
    """add_images 工具的输入参数"""
    draft_id: str  # 草稿ID
    image_infos: str  # 图片信息列表，JSON字符串
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_images，参数: {args.input}")
...
req_params_{generated_uuid}['image_infos'] = "{args.input.image_infos}"
...
req_{generated_uuid} = AddImagesRequest(**req_params_{generated_uuid})

resp_{generated_uuid} = await add_images(draft_{args.input.draft_id}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_images 调用成功")

        return Output(segment_ids=[], api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_images 时发生错误: {str(e)}"
        if logger:
