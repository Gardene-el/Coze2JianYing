...
class Input(NamedTuple):
    """add_keyframes 工具的输入参数"""
    draft_id: str  # 草稿ID
    keyframes: str  # 关键帧信息列表，JSON字符串


# Output 类型定义
class Output(NamedTuple):
    """add_keyframes 工具的输出参数"""
...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_keyframes，参数: {args.input}")
    ...
    try:
        ...
        # 生成 API 调用代码
        api_call = f"""
# API 调用: add_keyframes
# 时间: {time.strftime('%Y-%m-%d %H:%M:%S')}

# 构造 request 对象
req_params_{generated_uuid} = {{}}
req_params_{generated_uuid}['keyframes'] = "{args.input.keyframes}"
req_{generated_uuid} = AddKeyframesRequest(**req_params_{generated_uuid})

resp_{generated_uuid} = await add_keyframes(draft_{args.input.draft_id}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_keyframes 调用成功")

        return Output(api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_keyframes 时发生错误: {str(e)}"
        if logger:
