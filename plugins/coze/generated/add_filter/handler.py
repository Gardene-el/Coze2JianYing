...
def handler(args: Args[Input]) -> Dict[str, Any]:
    ...
    if logger:
        logger.info(f"调用 add_filter，参数: {args.input}")
...
resp_{generated_uuid} = await add_filter(draft_{args.input.draft_id}, req_{generated_uuid})
"""
        ...
        if logger:
            logger.info(f"add_filter 调用成功")

        return Output(api_call=api_call)._asdict()

    except Exception as e:
        error_msg = f"调用 add_filter 时发生错误: {str(e)}"
        if logger:
