...
1. **VideoSegment 参数** (7个):
   ...
   * 在本工具中映射为: `image_url`(material), `start/end`(target\_timerange)

2. **ClipSettings 参数** (6个):
   * `alpha`, `rotation`, `scale_x`, `scale_y`, `transform_x`, `transform_y`
   * 在本工具中映射为: `opacity`(alpha), `rotation`, `scale_x`, `scale_y`, `position_x`(transform\_x), `position_y`(transform\_y)
