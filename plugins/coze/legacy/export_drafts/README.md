...
#### draft\_ids (string | array | null)

* **描述**: 要导出的草稿ID，可以是单个UUID字符串或UUID字符串数组
* **单个草稿**: `"123e4567-e89b-12d3-a456-426614174000"`
* **多个草稿**: `["uuid1", "uuid2", "uuid3"]`
* **导出所有**: `null` (当export\_all=true时)
* **约束**: 必须是有效的UUID格式，或在export\_all模式下为null

#### remove\_temp\_files (boolean)
...
* **描述**: 是否导出目录中的所有草稿
* **默认值**: `false`
* **true**: 自动发现并导出所有草稿，忽略draft\_ids参数
* **false**: 按draft\_ids指定的草稿进行导出
...
1. **"草稿文件夹不存在"**: 检查draft\_id是否正确，草稿是否已被清理
