...
### 示例 6: 与 add\_captions 配合使用（完整工作流）
...
### make\_caption\_info → add\_captions
...
* Transform properties → position\_x, position\_y, scale, rotation, opacity
* Text style → font\_family, font\_size, font\_weight, font\_style, color
* Stroke → stroke\_enabled, stroke\_color, stroke\_width
* Shadow → shadow\_enabled, shadow\_color, shadow\_offset\_x/y, shadow\_blur
* Background → background\_enabled, background\_color, background\_opacity
* Layout → alignment
* Animations → intro\_animation, outro\_animation, loop\_animation
