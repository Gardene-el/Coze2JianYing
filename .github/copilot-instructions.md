...
这是一个完整的 Coze 到剪映工作流项目，整合了三个强关联但相对独立的子项目：

1. **Coze 插件** (`coze_plugin/`) - Coze 平台上的工具函数，处理参数和导出 JSON
2. **草稿生成器应用** (`app/`) - 包含 GUI 界面和 API 后端，将 JSON 转换为剪映草稿文件
3. **数据结构定义** (`data_structures/`) - 可复用的数据模型和接口定义

所有子项目在同一 GitHub 仓库中管理，便于维护和版本控制。
...
草稿生成器已发展为完整的应用，包含 GUI 界面和 API 后端：

1. **JSON 解析**: 解析 Coze 插件导出的 JSON 数据
2. **素材管理**: 从网络下载素材文件，统一存储管理
3. **草稿生成**: 调用 pyJianYingDraft 生成剪映草稿文件
4. **GUI 应用**: 提供友好的图形界面，支持日志查看和多标签页
5. **API 后端**: FastAPI 服务，支持本地服务和远程调用
6. **本地服务集成**: GUI 中可启动嵌入式 API 服务
7. **打包发布**: 可打包为 Windows exe 文件，独立分发
...
5. **make_*_info 系列工具** - 动态生成各类段配置 JSON
...
1. **GUI 应用** - 基于 Tkinter 的标签页架构图形界面
   - **草稿生成标签页** - 核心的草稿生成功能
   - **本地服务标签页** - 嵌入式 API 服务管理和 Coze API 配置
...
6. **草稿元数据管理** - 独立的 root_meta_info.json 生成
7. **打包脚本** - PyInstaller 构建 exe 文件
8. **GitHub Actions** - 自动化构建和发布
...
- **API 与 GUI 集成**: 支持多种部署模式（独立 API、独立 GUI、集成模式）
- **本地服务支持**: GUI 可启动和管理本地 API 服务，支持进程监控
...
- `gui/` - 图形界面模块
  ...
  - `local_service_tab.py` - 本地服务管理标签页
  - `log_window.py` - 日志窗口
- `api/` - API 路由模块
  - `router.py` - API 路由定义
