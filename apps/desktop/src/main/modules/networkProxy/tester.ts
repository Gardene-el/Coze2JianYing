...
export class ProxyConnectionTester {
  ...
  static async testConnection(
    url: string = ProxyConnectionTester.DEFAULT_TEST_URL,
    timeout: number = ProxyConnectionTester.DEFAULT_TIMEOUT,
  ...
  static async testProxyConfig(
    ...
    testUrl: string = ProxyConnectionTester.DEFAULT_TEST_URL,
  ): Promise<ProxyTestResult> {
    ...
    if (!config.enableProxy) {
      return ProxyConnectionTester.testConnection(testUrl)
    }
    ...
    try {
      ...
      const startTime = Date.now()
      const controller = new AbortController()
      const timeoutId = setTimeout(() => controller.abort(), ProxyConnectionTester.DEFAULT_TIMEOUT)
