    ...
    if (token) {
      ...
      t.on('exit', (code: number | null, signal: string | null) => {
        if (this.activeTunnel === t) {
          logger.warn(`cloudflared named tunnel exited (code ${code ?? 'null'}, signal ${signal ?? 'none'})`);
      ...
      logger.info(`Starting Cloudflare quick tunnel for http://127.0.0.1:${port} (protocol=${protocol})`);
      ...
      t.on('exit', (code: number | null, signal: string | null) => {
        if (this.activeTunnel === t) {
          logger.warn(`cloudflared quick tunnel exited (code ${code ?? 'null'}, signal ${signal ?? 'none'})`);
          this.activeTunnel = null;
          this.publicUrl = null;
        }
      });
      ...
      const publicUrl = await new Promise<string>((resolve, reject) => {
        ...
        t.once('exit', (code: number | null, signal: string | null) => {
          clearTimeout(timeout);
          if (readyTimer) clearTimeout(readyTimer);
          reject(new Error(`cloudflared 意外退出 (code ${code ?? 'null'}, signal ${signal ?? 'none'})`));
        });
      });
